...
Mesh::Mesh(const DualMatrix& V_, const Eigen::MatrixXi& F_)
    : V(V_), F(F_) {
...
void Mesh::computeAverageHeights(DualVector& heights) {
    ...
    for (int i = 0; i < EF.rows(); i++) {
        ...
        dual height0 = computeFaceHeight(F.row(face0), corner0);
        dual height1 = computeFaceHeight(F.row(face1), corner1);
...
dual Mesh::computeFaceHeight(const Eigen::RowVector3i& face, const int corner) {
    ...
    Dual3DVector crossProduct = edge.cross(v2 - v0);
    dual area = 0.5 * crossProduct.norm();
    dual height = (2.0 * area) / edge.norm();
